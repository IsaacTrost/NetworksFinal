...
class ThisNode():
    ...
    def create_merkle_tree(self, transactions):
        ...
        """
        if not transactions:
            return b'\x00' * 32  # Empty hash for empty transaction list
            
        # First, hash all transactions if they aren't already hashed
        leaves = [hashy(tx) if isinstance(tx, bytes) else tx for tx in transactions]
    ...
    def verify_merkle_proof(self, transaction, proof, merkle_root):
        """
    ...
    def get_merkle_proof(self, transactions, target_tx_index):
        """
        ...
        nodes = [hashy(tx) if isinstance(tx, bytes) else tx for tx in transactions]
    ...
    def get_merkle_root(self, transactions):
        ...
        Returns:
            bytes: The merkle root hash
        """
        return self.create_merkle_tree(transactions)
